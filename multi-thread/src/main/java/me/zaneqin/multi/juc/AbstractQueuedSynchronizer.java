...
public abstract class AbstractQueuedSynchronizer
     ...
     */
    // 阻塞的尾结点，每个新的节点进来，都插入到最后，形成一个链表
    private transient volatile Node tail;

    /**
     * The synchronization state.
     */
    // 关键字段，锁状态， 0表示没有被占用，大于0代表有线程持有锁
    // 这个值可以大于1， 锁可以重入， 每次重入都+1
     ...
     */
    // 如果tryAcquire(arg) = true 直接就结束了
    // 否则 acquireQueued方法会将线程压到队列中
    public final void acquire(int arg) {// arg = 1
        // 调用 tryAcquire(1)，直接成功就不需要进队列了
        // 对于公平锁的语义就是：本来就没有人持有锁，根本没必要进队列等待（又是挂起，又是等待被唤醒）
        if (!tryAcquire(arg) &&
                // tryAcquire没有成功， 需要把线程挂起，放入阻塞队列
     ...
     * @return the new node
     */
    // 此方法的作用是把线程包装成node，同时入队
    // 参数mode此时是Node.EXCLUSIVE, 独占模式
    private Node addWaiter(Node mode) {
        Node node = new Node(Thread.currentThread(), mode);
        // Try the fast path of enq; backup to full enq on failure
        // 以下几行代码想把当前node加入到链表的队尾
        Node pred = tail;

        // tail != null 队列不为空（tail==head 的时候，其实队列是空的）
        if (pred != null) {
            // 将当前的队尾节点，设置为自己的前驱
            node.prev = pred;
            // 用cas把自己设置为队尾，如果成功，tail = node了，这个节点成了新尾巴
            // todo cas 不明白
            if (compareAndSetTail(pred, node)) {
                // 进到这里说明设置成，node=tail,将自己与之前的队尾相连
                pred.next = node;
                // 线程入队了，可以返回了
                return node;
            }
        ...
        // 如果到了这里
        // 说明 pred == null 队列是空的或者cas 失败（有线程在竞争入队）
        enq(node);
        return node;
    }
     ...
     */
    // 采取自旋入队
    // 这个方法2个可能 ， 等待队列空位，或者有线程竞争入队
    // 自旋语义为：竞争一次竞争不到，就竞争多次，总会排到的（循环）
    private Node enq(final Node node) {
        for (;;) {
            ...
            if (t == null) { // Must initialize
                // 初始化head节点
                // head 和 tail初始化的时候都是null
                // cas设置head ， 有多线程竞争
                if (compareAndSetHead(new Node()))
                    // 给后面用， 这个时候head节点的waitStatus == 0
                    // 这个时候有了head，但是tail还是null，设置一下
                    // 把tail指向head 马上就要有线程来了，到时候tail就要被抢了
                    // 注意：没有return
                    // 之后会继续循环，进入else
                    tail = head;
            } else {
                // 下面几行和上一个addWaiter是一样的
                // 只是这个套在无限循环里，反正就是要排到队尾，排不上就是无限循环
                node.prev = t;
    ...
    //        && acquireQueued(addWaiter(Node.EXCLUSIVE), arg))
    //     selfInterrupt();
    // 下面这个方法，参数node，经过addWaiter(Node.EXCLUSIVE),此时已经进入到队尾
    // 注意一下，如果acquireQueued(addWaiter(Node.EXCLUSIVE), arg)) 返回true的话
    // 意味着要执行selfInterrupt()方法， 所有正常情况下是返回false的
    // 这个方法非常重要， 线程挂起和唤醒都在这个方法里
    final boolean acquireQueued(final Node node, int arg) {
        ...
        try {
            ...
            for (;;) {
                final Node p = node.predecessor();
                // p == head 说明当前节点虽然进入到阻塞队列，但是是阻塞队列的第一个， 因为他的前驱是head
                ...
                // 到这里说明上面的分支没有成功，
                // 要么当前node本来就不是队头， 要么就是tryAcquire(arg)没有抢到
                if (shouldParkAfterFailedAcquire(p, node) &&
                        parkAndCheckInterrupt())
                    interrupted = true;
            }
    ...
    final boolean transferForSignal(Node node) {
        ...
        // todo 关键操作设置前驱节点为Node.SIGNAL 如果 ws <= 0, 那么 compareAndSetWaitStatus 将会被调用，上篇介绍的时候说过，节点入队后，需要把前驱节点的状态设为 Node.SIGNAL(-1)
    ...
    public class ConditionObject implements Condition, java.io.Serializable {
         ...
         * </ol>
         */
        // 这个方法是可以被中断的，不可被中断的是{@link #awaitUninterruptibly}
        // 这个方法会阻塞，直到调用signal()方法(指signal()和signalAll(),下同)，或被中断
        public final void await() throws InterruptedException {
            ...
            // node.nextWaiter != null 怎么满足。
            // 我前面也说了 signal 的时候会将节点转移到阻塞队列，有一步是 node.nextWaiter = null，将断开节点和条件队列的联系。
            //
        ...
        // 而现在就是调用 parkNanos 方法来休眠指定的时间，醒来后判断是否 signal 调用了，调用了就是没有超时，否则就是超时了。
        // 超时的话，自己来进行转移到阻塞队列，然后抢锁。
